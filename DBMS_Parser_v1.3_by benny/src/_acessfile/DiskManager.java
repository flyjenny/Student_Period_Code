...
public class DiskManager {
	...
	public UnderlyingOperating $disk;
	private byte $buffer[];
	...
	public boolean WriteData(int byteNum,String data){
		...
		if(data.length()>(4096-relativeposition)){
			...
			result=handle.substring(0, relativeposition)+data+handle.substring(relativeposition+data.length(), 4096);
			WritePage(pageNum,result);
		}
		return true;
	}
	...
	public static void main(String args[]){
		...
		for(int i=0;i<64;i++)
			System.out.println(dm.Get_record()[i]);
