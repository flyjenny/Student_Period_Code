...
public class Police extends javax.swing.JFrame {
	...
	*/
	...
	public void get()
	{
		Class volun=null;
	try {
		volun=Class.forName("_searchInfo.PoliceClass");
	...
	Object obj = null;
	Object[] arg = new Object[] {45, "TOM","M"}; //自变量
	try {
	...
	$obj=obj;
	Field[] attrib=volun.getDeclaredFields();		//获得数据成员
	$attrib=attrib;

	Method[] method=volun.getDeclaredMethods();		//获得方法
	$method=method;
...
//		System.out.println($attrib.length);
		...
		String a=scan.next();		//获得public或者private属性
	//	System.out.println(a);
		if (a.equals("public"))		//public属性授权访问
		{
...
//		System.out.println(choose);
		...
		if (choose>=$attrib.length)
		{
			Scanner scan=new Scanner($method[choose-$attrib.length].toString());
			String a=scan.next();
			if (a.equals("public")) jTextPane2.setEditable(true);	//访问public的方法设置JTextPane可编辑
