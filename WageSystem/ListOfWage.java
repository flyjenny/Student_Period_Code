...
//	private WageSystem $system;	
	...
	PersonalWageInfo(int mark,int id,String name,double hours,double bonus,double fine,String remark){
		$ID=id;
		$name=name;
		$mark=mark;
		$hours=hours;
...
public class ListOfWage implements Serializable{
	private Map<Integer,PersonalWageInfo> WageList;		//记录工资的HashMap
	
	public ListOfWage(){
		WageList=new HashMap<Integer,PersonalWageInfo>();
			
	}
	
	//根据得到的ID和学生个人工资情况添加到HashMap中
	public void makeList(int id,PersonalWageInfo info){
		WageList.put(id, info);
	}
	public Map<Integer,PersonalWageInfo> getWageList(){
		return WageList;
	}

	public PersonalWageInfo search(int ID){
		return WageList.get(ID);
	}
	
	public Map<Integer,PersonalWageInfo> search(){
		return WageList;
	}
	
	public void output() throws RowsExceededException, WriteException, IOException{
		...
		while(iter.hasNext()){
			...
			jxl.write.Number labelA = new jxl.write.Number(0, i, ((PersonalWageInfo) entry.getValue()).getId());
