...
public class UserInfoList implements Serializable{
	Map<Integer, UserInfo> List;
	
	public static void main(String[] args){
		UserInfoList List = new UserInfoList();
		List.addUser(0, "inferno", "13761299817", "shanghai.sjtu", false, 1, "11111");
	...
	public UserInfoList(){
		...
		if (file.getUserInfoList()==null){
			List = new HashMap<Integer, UserInfo>();
		}
		else {
			List = file.getUserInfoList().getList();
		}
	}
	public Map<Integer, UserInfo> getList(){
		return List;
	}
	...
	public String getUserAddr(int ID){
		if (List.get(ID)==null)
			return null;
		return List.get(ID).getAddr();
	}
	public String getUserTel(int ID){
		if (List.get(ID)==null)
			return null;
	...
	public boolean isPoverty(int ID){
	...
	public boolean addUser(int ID, String name, String tele,
			String addr, boolean poverty, int type, String password){
		UserInfo user = new UserInfo(ID+"", name, tele, addr, poverty, type, password);
		if (user != null){
			List.put(ID, user);
			return true;
		}
	...
	public boolean editUser(int ID, String name, String tele,
			String addr, boolean poverty, int type, String password){
		UserInfo user = new UserInfo(ID+"", name, tele, addr, poverty, type, password);
		if (List.get(ID)!=null){
			if (password == null){
