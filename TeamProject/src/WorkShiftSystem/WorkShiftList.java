...
public class WorkShiftList implements Serializable
{
	...
	static int TimeOfWork[] = new int[]{3,4,3,4,3,4,3,4,3};
	static int WORKTOTAL = 9;
	...
	private void creatLabor(){
		int k = 0;
		for (int i = 0; i < freeTimeList.getAdmin().length; i++){
			LaborID[k] = freeTimeList.getAdmin()[i].ID;
			LaborName[k] = freeTimeList.getAdmin()[i].name;
			for (int j = 0; j < freeTimeList.getAdmin()[i].freetime.length; j++)
				Labor[k] += TimeOfWork[freeTimeList.getAdmin()[i].freetime[j]];
			k++;
		}
		for (int i = 0; i < freeTimeList.getEmp().length; i++){
			LaborID[k] = freeTimeList.getEmp()[i].ID;
			LaborName[k] = freeTimeList.getEmp()[i].name;
			for (int j = 0; j < freeTimeList.getEmp()[i].freetime.length; j++)
				Labor[k] += TimeOfWork[freeTimeList.getEmp()[i].freetime[j]];
			k++;
		}
	}
